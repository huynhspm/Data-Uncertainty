...
class UNetModel(nn.Module):
    ...
    d_cond_image: Optional[int] = None
    ...
    def forward(self, x, timesteps, extra):
        ...
        if self.d_cond_image and "label" not in extra:
            extra["label"] = torch.zeros(*x.shape[:1], self.d_cond_image, *x.shape[2:], device=x.device)

        if self.d_cond_image and "label" in extra:
            x = torch.cat([x, extra["label"]], dim=1)
