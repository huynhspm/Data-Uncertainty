...
def get_args_parser():
    ...
    parser.add_argument(
        "--condition",
        action="store_true",
        help="Train flow matching model with condition.",
    )
    parser.add_argument(
        "--compute_dice_iou",
        action="store_true",
        help="Whether to compute Dice and IoU in the evaluation loop.",
    )
    parser.add_argument(
        "--n_ensemble",
        default=1,
    ...
    parser.add_argument(
        "--mask_type",
        choices=["ensemble", "random"],
        default="ensemble",
        type=str,
        help="Use ensemble mask or random mask for each datapoint",
    )
    parser.add_argument(
        "--img_size",
        default=32,
        type=int,
        help="Size of image to resize for training",
    )
