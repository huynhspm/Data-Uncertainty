...
# only for continuos flow matching
def compute_dice_iou(solver: Solver,
    ...
    def decode(x):
        ...
        x = (x > 0.5).to(torch.int64)
        return x

    dice, iou = Dice(ignore_index=0).to(device), BinaryJaccardIndex().to(device)
    sample_partial = partial(
        solver.sample,
        time_grid=torch.tensor([0.0, 1.0], device=device),
        method=args.ode_method,
        atol=args.ode_options["atol"] if "atol" in args.ode_options else 1e-5,
        rtol=args.ode_options["rtol"] if "atol" in args.ode_options else 1e-5,
        step_size=args.ode_options["step_size"],
    ...
    for (samples, labels) in data_loader:
        ...
        for _ in range(args.n_ensemble):
            ...
            resamples.append(sample_partial(x_init=x_0, label=labels))
        
        resamples = torch.stack(resamples, dim=0).mean(dim=0)
        target, pred = decode(samples), decode(resamples)
        dice.update(target, pred)
        iou.update(target, pred)
...
def eval_model(
    ...
    for data_iter_step, (samples, labels) in enumerate(data_loader):
        samples = samples.to(device, non_blocking=True)
        labels = labels.to(device, non_blocking=True)
        fid_metric.update(samples if samples.shape[1] == 3 else samples.repeat(1, 3, 1, 1), real=True)
        ...
        if num_synthetic < fid_samples:
            ...
            fid_metric.update(synthetic_samples if synthetic_samples.shape[1] == 3 else synthetic_samples.repeat(1, 3, 1, 1), real=False)
    ...
    if args.compute_dice_iou:
        dice, iou = compute_dice_iou(solver, data_loader, device, args)
        metrics["dice"] = float(dice.detach().cpu())
        metrics["iou"] = float(iou.detach().cpu())
    return metrics
